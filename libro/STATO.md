...
| **L5** | I Tesi | **0/8** | solo `_OUTLINE.md`. |
| **L6** | La Spaccatura | **0/6** | solo `_OUTLINE.md`. |
| **L7** | La Soglia | **0/6** | solo `_OUTLINE.md`. |
...
- **Risultati pratici/schematici**, non muri di testo. Meno domande a raffica.
- **Mistero sepolto** anche dal narratore (il lettore collega, i POV no). Pieno di colpi di scena, mai banale/lento.
- **Gradazioni, mai on/off** ("più X" ≠ "zero Y").

## DOVE STA COSA (mappa rapida)
- **Prosa:** `MANOSCRITTO/LIBRO_1..7/` (un file per capitolo + `_OUTLINE.md` per libro).
- **Canone:** `BIBBIA/` — apri `BIBBIA/00_INDICE.md` per la mappa completa e le "fonti uniche".
- **Cruscotto:** questo `STATO.md` · decisioni in `DECISIONI.md` · buchi/lavori aperti in `DOMANDE_APERTE.md`.
- **Strumenti-schema (radice):** `timeline_cronologica.md`, `mappa_collegamenti.md`, `matrice_svolte.md`, `mondo_e_regioni.md`.
- **Manuale di lavoro + CANCELLO anti-leak:** `CLAUDE.md` (e copia `AGENTS.md`).

## MEMORIA DEL PERCHÉ (storico essenziale)
- La vecchia bozza di 60 capitoli fu **bruciata il 2026-06-26** (metodo sbagliato: troppi leak semantici). Il metodo per i nuovi capitoli è il CANCELLO (`CLAUDE.md`) + `BIBBIA/04_COME_SCRIVERE`.
- Il 2026-06-29 l'utente ha bocciato anche la 1ª riscrittura perché "lontana/finta/banale": da qui la **voce vicina** e la pulizia di tutto l'impianto teoria/documenti.
